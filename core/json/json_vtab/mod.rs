    ...
    fn count_elements(&self) -> Vec<usize> {
        ...
        for element in self.elements() {
            match element {
                ...
                PathElement::Key(range, _) => {
                    let key = &self.path()[range.start..range.end];
...
impl From<JsonPath> for InPlaceJsonPath {
    fn from(value: JsonPath) -> Self {
        Self {
            path: value.path().to_owned(),
            items_len: value.count_elements(),
        }
    }
}
