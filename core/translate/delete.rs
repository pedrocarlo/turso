...
pub fn translate_delete(
    ...
    let tbl_name_str = normalize_ident(tbl_name.name.as_str());

    // Check if this is a system table that should be protected from direct writes
    if !connection.is_nested_stmt()
        && !connection.is_mvcc_bootstrap_connection()
        && crate::schema::is_system_table(&tbl_name_str)
    {
        crate::bail_parse_error!("table {} may not be modified", tbl_name_str);
    }
...
fn bind_prepare_delete_plan(
    ...
    let tbl_name = normalize_ident(tbl_name_ast.as_str());
