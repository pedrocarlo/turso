...
/// Immutable planner inputs shared by every candidate evaluation in one join search.
#[derive(Clone, Copy)]
...
/// Mutable state for one join search: the access-method arena plus caches whose
/// lifetime is one candidate group.
...
/// The best B-tree access method for a table depends only on which tables are
/// already joined and how many rows they feed in, not on their order, so it can
/// be reused across join orders that share those inputs.
...
impl<'query, 'arena> JoinSearch<'query, 'arena> {
    ...
    fn tables_are_connected(&self, lhs_mask: &TableMask, rhs_table: usize) -> bool {
        self.where_terms
            .iter()
            .any(|term| term.table_mask.get(rhs_table) && term.table_mask.intersects(lhs_mask))
    }
    ...
    /// Drop arena entries added after `arena_start` that no surviving plan uses,
    /// and rewrite the surviving plans' access-method indices to the compacted
    /// positions. Keeps the arena from accumulating one entry per rejected candidate.
...
impl JoinSearch<'_, '_> {
    fn extend_plan(
        ...
        // The best B-tree access method depends only on the set of joined tables
        // and the input cardinality, both part of the cache key, so join orders
        // that share them can reuse the search result.
