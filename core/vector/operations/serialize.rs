...
pub fn vector_serialize(x: Vector) -> Value {
    match x.vector_type {
        VectorType::Float32Dense => Value::from_blob(x.bin_eject().to_vec_ext()),
        VectorType::Float64Dense => {
            let raw = x.bin_eject();
            let mut data: crate::alloc::Vec<u8> =
                crate::alloc::TursoVecExt::with_capacity(raw.len() + 1);
        ...
        VectorType::Float32Sparse => {
            ...
            let raw = x.bin_eject();
            let mut data: crate::alloc::Vec<u8> =
                crate::alloc::TursoVecExt::with_capacity(raw.len() + 5);
            data.extend_from_slice(&raw);
        ...
        VectorType::Float1Bit => {
            ...
            let mut blob: crate::alloc::Vec<u8> =
                crate::alloc::TursoVecExt::with_capacity(blob_size);
            let raw = x.bin_eject();
        ...
        VectorType::Float8 => {
            ...
            let mut blob: crate::alloc::Vec<u8> =
                crate::alloc::TursoVecExt::with_capacity(data.len() + 3);
