...
use crate::{turso_assert, turso_assert_eq, Buffer, Connection, LimboError, Result};
        ...
        #[cfg(not(feature = "omit_autovacuum"))]
        {
            ...
            match *auto_vacuum_mode {
                ...
                AutoVacuumMode::Full => {
                    ...
                    turso_assert!(root_page_num > 0, "[Pager - btree_create] Largest root page number cannot be 0 because that is set to 1 when creating the database with autovacuum enabled");
                    root_page_num += 1;
...
#[cfg(not(feature = "omit_autovacuum"))]
mod ptrmap {
    use crate::{storage::sqlite3_ondisk::MIN_PAGE_SIZE, turso_assert, LimboError, Result};
    ...
    pub fn entries_per_ptrmap_page(page_size: usize) -> usize {
        turso_assert!(
            page_size >= MIN_PAGE_SIZE as usize,
