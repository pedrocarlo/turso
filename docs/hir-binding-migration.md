...
- Plain aggregate calls with stable query-block identities and frozen result
  type facts. `COUNT(*)`, `DISTINCT`, and argument `ORDER BY` forms are explicit
  in HIR; nested aggregates are rejected during analysis.
...
1. Finish function forms: aggregate filters, ordered-set forms, windows, and
