...
|---|---|
| Trigger data-changing command programs | DML roots freeze matching trigger identities, and trigger predicates plus SELECT, INSERT, and UPDATE commands bind against NEW/OLD HIR sources. DELETE command bodies remain. Typed `Expr::Variable` nodes generated by its command rewrite belong to this checkpoint. |
