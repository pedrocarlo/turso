...
|---|---|
| Trigger command programs | DML roots freeze matching trigger identities and trigger predicates now bind against NEW/OLD HIR sources, but command bodies are not yet analyzed. Typed `Expr::Variable` nodes generated by the command rewrite belong to this checkpoint. |
...
- INSERT trigger identities are grouped by the write that can fire them.
  ...
  ordinary UPDATE triggers and matching `UPDATE OF` triggers. Trigger command
  programs and temp-schema catalog inputs remain later checkpoints.
...
- UPDATE trigger identities retain catalog order and include ordinary UPDATE
  ...
  Trigger command programs remain a later checkpoint.
...
- DELETE trigger identities retain catalog order and include only DELETE
  triggers for the target database. Trigger command programs remain a later
  checkpoint.
