...
|---|---|
| Trigger data-changing command programs | DML roots freeze matching trigger identities, and trigger predicates plus SELECT and INSERT commands bind against NEW/OLD HIR sources. UPDATE and DELETE command bodies remain. Typed `Expr::Variable` nodes generated by their command rewrite belong to this checkpoint. |
